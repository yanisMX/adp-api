...
async function getCategoriesFromUser(userId, wantedDate) {
	const statement = `SELECT c.id, c.name, c.budget, s.id as "spendingId", s.name as "spendingName", s.amount as "spendingAmount", s.recurrent as "spendingRecurrent", s.category_id as "spendingCategory" FROM category c LEFT JOIN spending s ON c.id = s.category_id AND (s.date >= $2 OR s.recurrent) WHERE c.user_id = $1 GROUP BY c.id, s.id`;
